...
class TextSplitter {
    ...
    recursiveSplit(text, separators, startPos) {
        ...
        if (separators.length > 0) {
            ...
            const joiner = this._config.keepSeparators
                ? ''
                : (sep !== ' ' && (sep.includes('\n') || sep.includes('\t')) ? ' ' : '');
    ...
    stripInlineSeparators(s) {
        if (this._config.keepSeparators || s.length === 0)
            return s;
    ...
    finalizeToChunks(text, startPos) {
        const chunks = [];
